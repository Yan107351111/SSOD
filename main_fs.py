...
data_path     = '../../datasets/dataset_horse_2020_08_03' # sys.argv[1] # data_path =  
folds_K       = 5
batch_size    = 400
batch_num     = 1000
label         = sys.argv[2] # label         ='horse'
bb_dict_path  = sys.argv[3] # bb_dict_path  = '../../SSOD/horse_bb_dict.p'
...
for fold in range(folds_K):
    ...
    ds_train = get_dataset_transformed(train_pathes, label)
    ds_valid = get_dataset_transformed(valid_pathes, label)
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        # print('\nTraining detector')
        # print(f'@ {time.time() - start_time}\n')
        det_trainer.fit(dlt, dlv, num_epochs = 1, start_epoch = epoch)
    ...
    image_source = f'/tcmldrive/Yann/datasets/dataset_horse_2020_08_03/fold_{fold}/positive_train'
    threshold = 0.3
    ds_train.output = 6
    detector.cuda()
    image_dict = {}
    ...
    print(f'final training score fold {fold}: {torch.mean(torch.stack(IOUs).float())}') 
    fold_train_scores.append(torch.mean(torch.stack(IOUs).float()))
        
    image_source = f'/tcmldrive/Yann/datasets/dataset_horse_2020_08_03/fold_{fold}/positive_valid'
    threshold = 0.3
    ds_train.output = 6
    detector.cuda()
    image_dict = {}
    ...
    for image in tqdm(list(image_dict)):
        ...
        iou = max(ious)
        IOUs.append(iou>threshold)
    print(f'final validation score fold {fold}: {torch.mean(torch.stack(IOUs).float())}') 
    fold_valid_scores.append(torch.mean(torch.stack(IOUs).float()))

print(f'final training scores: {fold_train_scores}') 
print(f'mean training score: {torch.mean(torch.tensor(fold_train_scores))}') 
print(f'final validation scores: {fold_valid_scores}') 
print(f'mean validation score: {torch.mean(torch.tensor(fold_valid_scores))}') 
